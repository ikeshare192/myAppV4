...
st.subheader("**1. Selct the file type below and upload the training data**")
...
def get_true_checks(check_dict):
    selected = []
...
#then go to the left-side bar and train an algorithm**") 
    ...
    if choose_file_type == "CSV":
        ...
        #upload file
        df,columns = upload_csv()
        targ_var = choose_target(df.columns)
        st.write(f'You have chosen to predict** {targ_var}** now go train an algorithm')
        for i in columns:
            if i == str(targ_var):
    ...
    if choose_file_type == "Excel" or choose_file_type == "CSV":
        st.checkbox("would you like to select your training colums?")
    else:
        pass
